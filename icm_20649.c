/***************************************************************************//**
 * @file
 * @brief Driver for the Invensense ICM20649 6-axis motion sensor
 *******************************************************************************
 * # License
...
/**************************************************************************//**
* @defgroup ICM20649 ICM20649 - 6-axis Motion Sensor Driver
* @{
* @brief Driver for the Invensense ICM20649 6-axis motion sensor
******************************************************************************/
...
uint32_t ICM20649_wakeOnMotionITEnable(bool enable, uint8_t womThreshold, float sampleRate)
{
  if ( enable ) {
    ...
    ICM20649_sensorEnable(true, false, false);

    /* Set sample rate */
    ICM20649_sampleRateSet(sampleRate);

    /* Set the bandwidth to 1210Hz */
    ICM20649_accelBandwidthSet(ICM20649_ACCEL_BW_1210HZ);

    /* Accel: 2G full scale */
    ICM20649_accelFullscaleSet(ICM20649_ACCEL_FULLSCALE_2G);

    /* Enable the Wake On Motion interrupt */
    ICM20649_interruptEnable(false, true);
    UTIL_delay(50);

    /* Enable Wake On Motion feature */
    ICM20649_registerWrite(ICM20649_REG_ACCEL_INTEL_CTRL, ICM20649_BIT_ACCEL_INTEL_EN | ICM20649_BIT_ACCEL_INTEL_MODE);

    /* Set the wake on motion threshold value */
    ICM20649_registerWrite(ICM20649_REG_ACCEL_WOM_THR, womThreshold);

    /* Enable low power mode */
    ICM20649_lowPowerModeEnter(true, false, false);
  } else {
    /* Disable Wake On Motion feature */
    ICM20649_registerWrite(ICM20649_REG_ACCEL_INTEL_CTRL, 0x00);

    /* Disable the Wake On Motion interrupt */
    ICM20649_interruptEnable(false, false);
...
uint32_t ICM20649_i2cInit(void)
{
  ...
  GPIO_PinModeSet(sclPin, gpioModePushPull, 0);    /* Clock */
